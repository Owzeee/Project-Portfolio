...
app.post('/login', async (req, res) => {
    try {
        // First, attempt to find the user in the software_providers table
        db.query('SELECT *, "softwareProvider" as role FROM software_providers WHERE email = ?', [req.body.email], async (error, providerResults) => {
            ...
            if (providerResults.length > 0) {
                ...
                // If not found, then check in the clients table
...
app.post('/reset-password-request', async (req, res) => {
    try {
        ...
        // Check user existence in both tables and get user details
        ...
        const mailOptions = {
            from: 'licensemaster220@gmail.com', // Replace with your actual email
            to: userEmail,
...
function generateSerialNumber() {
  ...
  // Endpoint to generate a serial number
  app.post('/api/generate-serial', (req, res) => {
    ...
    const createdBy = req.session.user.id; // Assuming the user ID is stored in the session
  ...
  // Endpoint to associate a serial number with a product and client
  ...
  // Endpoint to enable/disable a serial number
  ...
  // Endpoint to notify a client through email
  ...
  app.get('/api/recent-activities/clients', (req, res) => {
    try {
        // Fetch recent client activities from the database
...
// Endpoint to get recent developer activities
app.get('/api/recent-activities/developer', (req, res) => {
    try {
        // Fetch recent activities for developers from the database
...
// Endpoint to add a recent activity
...
app.post('/api/add-activity', (req, res) => {
    ...
    // Ensure that the category is 'developer' since it's the only existing category
    if (category !== 'developer') {
        return res.status(400).json({ status: 'error', message: 'Invalid category' });
    }

    // Insert the new activity into the 'dev_activity_log' table with the current timestamp
    db.query(
...
app.get('/api/active-licenses-count', async (req, res) => {
    try {
        // Replace with actual query to count active licenses
...
app.get('/api/my-serial-numbers', (req, res) => {
    ...
    // Query to select serial numbers created by the logged-in user
...
app.post('/api/licenses/:id/stop-renew', async (req, res) => {
  const licenseId = req.params.id;
  const updateQuery = 'UPDATE licenses SET isRenewing = 0 WHERE id = ?'; // Assuming you have a field like 'isRenewing'
...
app.post('/generate-serial-number', (req, res) => {
    ...
    const serialNumber = crypto.randomBytes(20).toString('hex');
    const { productName, expirationDate } = req.body;
    const createdBy = req.session.user.id; // Retrieve the ID of the logged-in user
...
// Endpoint to list all active serial numbers
...
app.post('/update-serial-details', (req, res) => {
    ...
    // First, check if the logged-in user created this serial number
    db.query('SELECT * FROM serial_numbers WHERE id = ? AND created_by = ?', [id, userId], (error, results) => {
        ...
        if (results.length === 0) {
            // The user did not create this serial number, or it doesn't exist
